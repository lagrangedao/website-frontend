  ...
  <div id="hardware" v-loading="forkLoad">
    <div v-if="props.renewButton !== 'renew'">
      <div class="space-hard"
           v-if="props.renewButton === 'setting' && props.listdata.activeOrder && props.listdata.activeOrder.config">
          ...
          <el-col :md="24" :lg="6" class="hardware-left">
            <div class="sleep_style">
              ...
              <p class="p-4">Building something cool as a side project? Apply for a
                <span>
                  <svg class="text-xs mr-1.5" xmlns="http://www.w3.org/2000/svg"
            ...
            <el-row :gutter="25" class="space_hardware_list" v-for="(item, index) in hardwareOptions" :key="index">
              ...
              <el-col :xs="24" :sm="12" :md="12" :lg="6" :xl="6" v-for="(ol, o) in item.list" :key="o">
                <el-card class="box-card"
                  ...
                  <div class="abo"
                       v-if="props.renewButton === 'setting' && props.listdata.activeOrder && props.listdata.activeOrder.config && props.listdata.activeOrder.config.name === ol.hardware_name && (props.listdata.activeOrder.ended_at === null || (props.listdata.activeOrder.ended_at !== null && props.listdata.activeOrder.ended_at > Math.floor(Date.now() / 1000))) && props.listdata.status !== 'Stopped'">
                    <svg t="1678084765267" class="icon" viewBox="0 0 1024 1024" version="1.1"
    ...
    <el-dialog custom-class="sleep_body" @close="close" v-model="sleepVisible" title="Confirm hardware update"
               :width="dialogWidth">
      <div v-loading="hardwareLoad">
        ...
        <div class="sleep_style">
          <div>
            ...
            <div class="time flex">
              <el-input-number v-model="ruleForm.usageTime" :min="24"
                               :max="sleepSelect.hardware_type.toLowerCase() === 'gpu' ? 168:336" :precision="0"
                               :step="1" controls-position="right"/> &nbsp; hours
            </div>
          </div>
          <div>
            <div class="title_tip flex">
            ...
            <div class="time flex">
              <el-select v-model="sleepSelect.regionValue" class="m-region" placeholder="Region">
                <el-option v-for="item in sleepSelect.regionOption" :key="item.value" :label="item.label"
                           :value="item.value"/>
              </el-select>
            </div>
          </div>
          <div>
            <div class="title_tip flex">
              <div class="flex">
                ...
                <el-popover placement="top-start" :width="200" trigger="hover"
                            content="If the task isn't successfully taken by the CP within the specified time, we will consider it as failed and refund your tokens.">
                  <template #reference>
                    <div>
                      <svg class="" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                           aria-hidden="true" focusable="false" role="img" width="1em" height="1em"
            ...
            <div class="time flex">
              ...
              <el-select v-model="ruleForm.sleepTime" class="m-2" placeholder="Select" size="small">
                <el-option v-for="item in ruleForm.sleepTimeOption" :key="item.value" :label="item.label"
                           :value="item.value"/>
              </el-select>
            </div>
            <p class="p-1 p-small" v-if="sleepSelect.hardware_type.toLowerCase() === 'gpu' && ruleForm.sleepTime < 20">
              For GPU tasks, we recommend that you choose a longer wait time to ensure that the CP can complete the task
              within your desired time frame</p>
          </div>
          <el-divider/>
