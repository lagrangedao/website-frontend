  ...
  <div class="space-hard">
    <el-row class="space_hardware" :gutter="30">
      <el-col :xs="24" :sm="24" :md="24" :lg="6" :xl="6">
        ...
        <p class="p-4">
          You'll be billed for uptime on a per minute basis.<br> View usage in your
      ...
      <el-col :xs="24" :sm="24" :md="24" :lg="18" :xl="18">
        ...
        <el-row :gutter="25" class="space_hardware_list" v-for="(item,index) in hardwareOptions" :key="index">
          <el-col :xs="24" :sm="12" :md="12" :lg="6" :xl="6" v-for="(ol, o) in item.options.concat(item.options_setting)" :key="o">
            <el-card class="box-card" :class="{'active': props.listdata.hardware === ol.value, 'is-disabled':ol.value >= 6}" @click="sleepChange(ol)">
              <div class="abo" v-if="props.listdata.hardware === ol.value">
    ...
    <el-dialog custom-class="sleep_body" v-model="sleepVisible" title="Confirm hardware update" :width="dialogWidth">
      <div v-loading="hardwareLoad">
        ...
        <div class="sleep_style">
          ...
          <div class="time flex">
            <el-input-number v-model="ruleForm.usageTime" :min="1" :max="168" controls-position="right" /> &nbsp; hours
          </div>
          <el-divider />
          <p class="p-1">Make sure to follow
            <router-link to="">this guide</router-link> to ensure your Space takes advantage of the improved hardware.</p>
          <p class="p-1">You can switch to a different hardware at any time. Monitor your usage in real time from your
            <router-link to="">billing settings</router-link>.</p>
        </div>
      </div>
...
export default defineComponent({
  ...
  setup (props, context) {
    ...
    const space_contract_address = process.env.VUE_APP_HARDWARE_ADDRESS
    const space_contract = new system.$commonFun.web3Init.eth.Contract(SpaceHardwareABI, space_contract_address)

    async function hardwareFun () {
      hardwareLoad.value = true
      try {
        let gasLimit = await space_contract.methods
          .makePayment(route.params.wallet_address, route.params.name, Number(sleepSelect.value.value), Number(ruleForm.usageTime))
          .estimateGas({ from: store.state.metaAddress })

        const tx = await space_contract.methods
          .makePayment(route.params.wallet_address, route.params.name, Number(sleepSelect.value.value), Number(ruleForm.usageTime))
    ...
    async function hardwareHash (tx_hash) {
      ...
      fd.append('hardware', sleepSelect.value.value)
      fd.append('vcpu', sleepSelect.value.vCPU)
      fd.append('memory', 123456)
      fd.append('duration', 123456)
      fd.append('price_per_hour', ruleForm.usageTime)
    ...
    function sleepChange (row) {
      if (props.listdata.hardware === row.value || row.value >= 6) return false
