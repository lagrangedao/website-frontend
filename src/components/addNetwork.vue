...
export default defineComponent({
  ...
  setup (props, context) {
    ...
    async function uploadFun () {
      ...
      const uploadRes = await system.$commonFun.sendRequest(`${process.env.VUE_APP_RPCAPI}v1/files`, 'post', fd)
    ...
    async function submitAdd (formEl) {
      ...
      await dataAddRef.value.validate(async (valid, fields) => {
        if (valid) {
          ...
          const addNetRes = await system.$commonFun.sendRequest(`${process.env.VUE_APP_RPCAPI}v1/nodes`, 'post', params)
